...
// 依赖：官方 iobject-js SDK（import 'iobject-js'，由 Vite 从 third_party/IObject/js 即时转译）、window.Shin.binary。
import { IObjectClient, RemoteObject } from 'iobject-js';

const $ = (id) => document.getElementById(id);
let client = null;               // IObjectClient
...
function ro(addr) { return new RemoteObject(client, addr); }
...
async function doConnect() {
  try {
    ...
    client = await IObjectClient.connect(url, opts);
...
log('调试工具已加载 v5 [' + modeName + (MODE === 'ws' ? ' ' + WS_URL : '') + ']');
